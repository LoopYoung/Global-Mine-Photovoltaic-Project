# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 18:47:37 2024
...
#function for calculating the optimal tilt
def calculate_tilt(latitude):
    ...
    return tilt
#function for calculating the solar altitude and azimuth at 3:00PM at winter solstice
...
#function for calculating packing factor
...
MineLocation = pd.read_csv("Your mine data with coordinates")
for i in tqdm(range(len(MineLocation)), desc='Processing locations', position=0, mininterval=300):
    ...
    try:
        ...
        #Calculate energy generation(kwh)/area/year
    ...
    MineLocation.iloc[i:i + 1].to_csv(os.path.join("outputfile.csv"), mode='a', header=False, index=False)
